"""
Strategy A Server — fake data center
====================================

Receives raw-audio batches from edge clients over MQTT, does everything
Strategy A puts on the server side:

    decompress → STFT (the defining Strategy A trait) → pool →
    train a round when the pool crosses --train-trigger-samples →
    evaluate on the held-out fold-10 test set → broadcast weights

MQTT messages arrive on paho's network thread; the callback only enqueues
raw payloads and the main loop does all decompression/STFT/training
(producer-consumer), so a long model.fit never blocks the socket.

Warm-starts from models/audio_cnn_urbansound8k.keras when present,
otherwise trains the AudioCNN from scratch.

Spawned by scripts/run_strategy_a.py — see it for the CLI contract:
it greps the log for READY_MARKER before starting clients, and parses
{run-dir}/server.json (rounds[], bytes_broadcast_total,
per_client_bytes_total) for its summary.
"""
...
READY_MARKER = "Server ready. Waiting for edge batches"  # grepped by the harness
BATCH_SIZE = 32
LR = 1e-3
...
class StrategyAServer:
    def __init__(self, args: argparse.Namespace):
        ...
        # Raw payloads cross from paho's network thread through this queue;
        # all heavy work (decompress/STFT/fit) stays on the main thread.
    ...
    def _on_data(self, topic: str, payload: bytes) -> None:
        """Runs on paho's thread — enqueue only, return fast."""
    ...
    def ingest(self, topic: str, payload: bytes) -> None:
        ...
        # Server-side STFT — the compute Strategy A deliberately keeps
        # off the edge device.
        specs = self.stft.process_batch(audio[keep])[..., np.newaxis]
        self.X_pool.append(specs.astype(np.float32))
        ...
        print(f"Batch from {node_id}: {len(labels)} samples "
    ...
    def run_round(self) -> None:
        ...
        print(f"=== Round {rnd} done: test_acc={test_acc:.3f}, "
              ...
              flush=True)
        self.write_report()  # incremental, so a crash never loses finished rounds
    ...
    def run(self) -> int:
        ...
        # SIGTERM: drain whatever already arrived (bytes must be counted),
        # but don't start new training rounds — the harness only waits 15s.
        while True:
            try:
